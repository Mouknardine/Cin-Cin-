...
export function ChampSeancesDuFilm(): React.JSX.Element {
  ...
  const charger = useCallback(() => {
    ...
    client
      ...
      .then((r) => {
        /* L'ordre du programme (voir sanity/salles.ts). Sanity
           s'arrête à la minute — il rangerait « Hall-Bar » avant
           « Salle 1 », et la deuxième séance d'une salle avant la
           première de l'autre. */
        setSeances(ordonnerSeances(r.seances ?? []))
        setPublie(r.publie)
      })
      .catch(() => {
        setSeances([])
